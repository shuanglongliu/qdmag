...
if __name__ == "__main__":
    ...
    h_tmin = transform_O(h_ex + h_ani + h_zee, eigen_eff)
    ...
    eigen = eigen_simple(h_eff)
    P0_eff = get_Pe(eigen.eigenvalues, T)

    # Time evolution
    start = time.time()
    tmax, P_eff = evolve_P_stairs(P0_eff, nu0, symmetric, tmin, tmax, deltat, Bt_params, T, h0_eff, Mz_eff, nt_save, eigen.dim)
    end   = time.time()
