...
def generate_combined_quadrature(lebedev_degree=31, n_gamma_points=12, save_file=True):
    ...
    print(f"Generated {len(weights)} quadrature points:")
    print(f"  - {sphere_points[0].shape[0]} Lebedev points (degree {lebedev_degree})")
    print(f"  - {len(gamma_points)} Gauss-Legendre points for γ rotation")

    # Save the quadrature points and weights to a file
    if save_file:
        with open(f'lebedev_gauss_legendre_points_{lebedev_degree}_{n_gamma_points}.txt', 'w') as f:
            f.write("# Lebedev-Gauss-Legendre quadrature points and weights\n")
        ...
        print(f"Quadrature points and weights saved to 'lebedev_gauss_legendre_points_{lebedev_degree}_{n_gamma_points}.txt'")
