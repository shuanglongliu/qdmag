...
def set_up_the_effective_system_args(selected_states, *args):
    """
    Obtain effective operators on the basis of selected states. 
    All the operators are on the perturbed basis, i.e. the eigenstates of h_ex + h_zee(B = 1e-4 T).

    Input: 
        selected_states: a list of indices of the selected states counting from zero.
        args: a list of operators in the whole Hilbert space.
    """
    ...
    # np.complex64 is not enough to capture the energy differences between the nearly degenerate states (which are indeed degenerate without perturbation).
    args_eff = [np.zeros((n, n), dtype=np.complex128) for i in range(n_args)]

    # Construct the effective operators
    for i in range(n):
        for j in range(n):
            for i_arg in range(n_args):
                args_eff[i_arg][i, j] = args[i_arg][selected_states[i], selected_states[j]]
